...
discussing the benefits and risks of each one, and ultimatley choosing the
system that appropriatley meets all of the performance requirements.
...
In this task we used the PID Compensator architectire, as discussed in class,
...
furthermore states that $\frac{K_1}{K_2} = 1$. As in the previous Tasks, our 
...
In Conclusion, this PID Compensator unfortunatley did not meet all requirements
...
Task 4. Specifically, the PID Compensator did not meet the required settling
time (Less than 250 ms). From the table above, the PID Compensator could only
acheive a settling time of 3.82 s, at a rather high K value. However the 
percent overshoot was practically nonexistent, and so if at some later point
the specifications prefer extremley low overshoot percentage over settling time,
this would be the compensator to use.
