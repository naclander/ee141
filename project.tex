...
for the Hard Disk Drive system. Not all controllers met the neccessary
...
Sensor (Task 4), and the Refined Arm Model design (Task 6). However, the extra
specification of a resonant peak outside the 3-dB bandwidth was not met for the
Refiend Arm Model design.
...
The closed loop transfer fucntion of the entire system, as mentioned in Task 4,
came out to be:

$$HDD(s) = \frac{F(s)G_1(s)G_2(s)}{1 + F(s)G_1(s)G_2(s)H(s)}$$

It was shown in Task 4 that all requirements were met. The table showing the
perormance results is shown below for convenience:
...
acheive a settling time of 165 ms, although lowering the Gain Margin. If in the
